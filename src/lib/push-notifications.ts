...
export async function subscribeUserToPush() {
  ...
  try {
    ...
    // IF NO REGISTRATION FOUND OR NOT ACTIVE, ATTEMPT MANUAL REGISTRATION
    if (!registration || !registration.active) {
      console.log("🛠️ Attempting manual Service Worker registration for /sw.js...");
      try {
        // We explicitly register /sw.js which is generated by next-pwa
        registration = await navigator.serviceWorker.register('/sw.js', { scope: '/' });
        console.log("✅ Manual SW registration triggered:", registration.scope);
      } catch (regErr) {
        console.error("❌ Manual SW registration failed:", regErr);
      }
    }

    // Still wait for .ready to ensure it's fully initialized
    console.log("⏳ Waiting for .ready with 20s timeout...");
    const swTimeout = new Promise((_, reject) => {
      setTimeout(() => reject(new Error("Service Worker registration timed out (20s). Please check if your browser allows workers.")), 20000);
    });
    ...
    if (!registration || !registration.active) {
      console.warn("⚠️ Registration found but maybe not active yet. Waiting 1.5s for activation...");
