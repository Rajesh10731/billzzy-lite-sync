...
const MobileProductCard: FC<MobileProductCardProps> = React.memo(
  ({ product, isSwiped, onSwipe, onEdit, onDelete, updatedProductInfo }) => {
    const controls = useAnimationControls();
    ...
    const isSynced = product.source === 'MASTER';
    ...
    return (
      <div
        className={`relative w-full bg-gray-200 rounded-lg overflow-hidden shadow-sm ${isLowStock ? 'ring-2 ring-red-500' : ''}`}
      >
        <div className="absolute inset-y-0 right-0 flex" style={{ width: ACTION_WIDTH }}>
          <button
          ...
          <button
            ...
            className="w-1/2 h-full flex flex-col items-center justify-center bg-red-500 text-white transition-colors hover:bg-red-600"
          >
        ...
        >
          {product.image ? (
            ...
            <button
              ...
              className="w-14 h-14 flex-shrink-0 flex flex-col items-center justify-center bg-gray-100 rounded-md hover:bg-gray-200 transition-colors"
            >
...
const DesktopProductTable: FC<{
  products: Product[];
  onEdit: (p: Product) => void;
  onDelete: (id: string) => void;
  updatedProductInfo: UpdateInfo | null;
}> = ({ products, onEdit, onDelete, updatedProductInfo }) => (
            ...
            return (
              <motion.tr layout key={p.id} className={isLowStock ? 'bg-red-50' : ''}>
                ...
                <td className="px-3 py-2 text-sm text-green-600 font-medium">
                  {p.profitPerUnit ? formatCurrency(p.profitPerUnit) : '-'}
                </td>
                <td className="px-3 py-2 text-right">
                  <div className="flex justify-end gap-4">
                    ...
                    {p.source === 'MASTER' ? (
                      <span
                        className="text-gray-400 flex items-center gap-1 text-xs font-medium cursor-help"
                        title="Managed by Billzzy"
                      >
                        <Lock size={14} /> Managed by Billzzy
                      </span>
                    ) : (
                      <>
                        <button
                          onClick={() => onEdit(p)}
                          className="text-[#5a4fcf] hover:text-[#4a3fb5] flex items-center gap-1"
                        >
...
type ProductFormState = Omit<
  ProductFormData,
  'quantity' | 'buyingPrice' | 'sellingPrice' | 'gstRate' | 'lowStockThreshold' | 'profitPerUnit'
> & {
  quantity: number | '';
  buyingPrice?: number | '';
  sellingPrice?: number | '';
...
const ProductFormModal: FC<ProductFormModalProps> = ({ product, onSave, onClose }) => {
  ...
  const showProfitCalculation =
    Number(formData.profitPerUnit) > 0 && profitCalculations.quantity > 0;
      ...
      >
        <div className="bg-gradient-to-r from-[#5a4fcf] to-[#7b68ee] px-4 py-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-indigo-100 text-xs mt-0.5">
              {product?.id ? 'Update product information' : 'Fill in the details below'}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {isScannerOpen ? (
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full px-4 py-2 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 font-medium transition-colors text-sm"
              >
                Cancel Scan
              </button>
            </div>
            ...
            <>
              <div className="space-y-1.5">
                ...
                <div
                  className="w-full h-32 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center cursor-pointer hover:border-[#5a4fcf] transition-colors"
                  onClick={() => !isSubmitting && fileInputRef.current?.click()}
                >
                  <input
                    type="file"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full border-2 border-gray-200 px-3 py-2 pr-12 rounded-xl focus:border-[#5a4fcf] focus:ring-2 focus:ring-[#5a4fcf]/20 transition-all outline-none font-mono text-xs"
                    value={formData.sku || ''}
                    onChange={handleInputChange}
                    disabled={isSubmitting}
                  />
                  <button
                    ...
                    disabled={isSubmitting}
                    className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center justify-center w-9 h-9 bg-gradient-to-r from-[#5a4fcf] to-[#7b68ee] text-white rounded-xl hover:from-[#4a3fb5] hover:to-[#6b58de] shadow-md shadow-[#5a4fcf]/20 transition-all active:scale-95"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-gray-700">
                    {isEditing ? 'Current Quantity' : 'Quantity'}
                  </label>
                  <input
                    type="number"
                    name="quantity"
                    placeholder="e.g., 50"
                    value={isEditing ? product.quantity : formData.quantity}
                    readOnly={isEditing}
                    onChange={!isEditing ? handleInputChange : undefined}
                    className={`w-full border-2 border-gray-200 px-3 py-2 rounded-xl focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all outline-none text-sm ${isEditing ? 'bg-gray-100 cursor-not-allowed' : ''}`}
                ...
                <div className="space-y-1.5">
                  <label
                    ...
                    title="Set a custom alert when quantity falls to this level. Leave blank for default."
                  >
                    Low Stock Alert <Info className="w-3 h-3 text-gray-400 cursor-help" />
                  </label>
                  <input
                    type="number"
              ...
              {isEditing && (
                <div className="p-3 bg-indigo-50 rounded-xl border border-indigo-200 space-y-2">
                  <label className="text-xs font-medium text-indigo-800 block">
                    Stock Adjustment
                  </label>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 text-center">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setStockAdjustment(e.target.value === '' ? '' : Number(e.target.value))
                        }
                        className="w-full mt-1 border-2 text-center border-gray-200 px-3 py-2 rounded-xl focus:border-[#5a4fcf] focus:ring-2 focus:ring-[#5a4fcf]/20 transition-all outline-none text-sm"
                        disabled={isSubmitting}
                      />
                    </div>
                    <div className="text-2xl text-gray-400 font-light">=</div>
                    <div className="flex-1 text-center">
                      <p className="text-xs text-gray-600">New Total Stock</p>
                      <div className="mt-1 font-bold text-lg text-[#5a4fcf]">
                        {(product.quantity || 0) + (Number(stockAdjustment) || 0)}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className={`grid grid-cols-1 ${!isGstInclusive ? 'sm:grid-cols-2' : ''} gap-3`}>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-gray-700">
                    {isGstInclusive ? 'Total Price (incl. GST)' : 'Selling Price (excl. GST)'}
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full border-2 border-gray-200 pl-7 pr-3 py-2 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none text-sm"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        name="gstRate"
                        placeholder="e.g., 18"
                        className="w-full border-2 border-gray-200 px-3 py-2 pr-10 rounded-xl focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all outline-none text-sm"
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-center flex-1">
                      ...
                      <div className="text-green-700 font-semibold text-xs mt-0.5">
                        {formatCurrency(profitCalculations.profitPerUnit)}
                      </div>
                    </div>
                    <div className="text-xl text-gray-400 font-light px-2">×</div>
                    <div className="text-center flex-1">
        ...
        <div className="bg-gray-50 px-4 py-3 flex justify-end gap-2.5 border-t">
          <button
            onClick={onClose}
            disabled={isSubmitting}
            className="px-5 py-2 bg-white border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 font-medium transition-colors text-sm disabled:opacity-50"
          >
            Cancel
          </button>
          <button
...
const Inventory: FC = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeFilter === 'low') {
      result = result.filter(
        (p) => p.quantity > 0 && p.quantity <= (p.lowStockThreshold ?? LOW_STOCK_THRESHOLD)
      );
    ...
    async (e: ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = async (event: ProgressEvent<FileReader>) => {
        ...
        try {
          ...
          if (!response.ok)
            throw new Error((await response.json()).message || 'Failed to upload products');
  ...
  const handleSaveProduct = useCallback(
    async (productData: ProductFormData, imageFile: File | null) => {
      const isEditing = !!productData.id;
      const originalQuantity = isEditing
        ? products.find((p) => p.id === productData.id)?.quantity || 0
        : 0;
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || `Failed to ${isEditing ? 'update' : 'create'} product`
          );
        }
        ...
        if (isEditing) {
          ...
          setProducts((prevProducts) =>
            prevProducts.map((p) => (p.id === productData.id ? savedProduct : p))
          );

          const quantityChange = savedProduct.quantity - originalQuantity;
          if (quantityChange !== 0)
            setUpdatedProductInfo({ id: savedProduct.id, change: quantityChange });
        } else {
    ...
    if (sessionStatus === 'unauthenticated')
      return (
        <div className="text-center h-64 flex flex-col justify-center items-center bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-500">Log in to manage your inventory.</p>
        </div>
      );
      ...
      return (
        <div className="text-center h-64 flex flex-col justify-center items-center bg-gray-50 rounded-lg">
          ...
          <button
            ...
            className="mt-4 text-[#5a4fcf] font-medium underline"
          >
            Clear all filters
          </button>
        </div>
            ...
            {rowVirtualizer.getVirtualItems().map((virtualItem) => (
              ...
              >
                <MobileProductCard
                  ...
                  isSwiped={swipedProductId === filteredProducts[virtualItem.index].id}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3.5 space-y-3.5">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <button
              ...
              className={`relative group flex flex-col items-center justify-center p-2 rounded-xl border-2 transition-all duration-300 active:scale-95 text-center h-24 ${
                activeFilter === 'all'
                  ? 'bg-indigo-50 border-indigo-500'
                  : 'bg-white border-gray-100 hover:border-gray-200 hover:bg-gray-50'
              }`}
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center mb-1 transition-colors ${activeFilter === 'all' ? 'bg-indigo-500 text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'}`}
              >
                <Package className="w-4 h-4" />
              </div>
              <span
                className={`text-[10px] font-bold uppercase tracking-wide ${activeFilter === 'all' ? 'text-indigo-500' : 'text-gray-400'}`}
              >
                All Stock
              </span>
              <p
                className={`text-lg font-extrabold ${activeFilter === 'all' ? 'text-gray-900' : 'text-gray-700'}`}
              >
                {stats.totalProducts}
              </p>
              {activeFilter === 'all' && (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center mb-1 transition-colors ${activeFilter === 'low' ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'}`}
              >
                <AlertTriangle className="w-4 h-4" />
              </div>
              <span
                className={`text-[10px] font-bold uppercase tracking-wide ${activeFilter === 'low' ? 'text-orange-500' : 'text-gray-400'}`}
              >
                Low Stock
              </span>
              <p
                className={`text-lg font-extrabold ${activeFilter === 'low' ? 'text-gray-900' : 'text-gray-700'}`}
              >
                {stats.lowStock}
              </p>
              {activeFilter === 'low' && (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center mb-1 transition-colors ${activeFilter === 'out' ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'}`}
              >
                <AlertCircle className="w-4 h-4" />
              </div>
              <span
                className={`text-[10px] font-bold uppercase tracking-wide ${activeFilter === 'out' ? 'text-red-500' : 'text-gray-400'}`}
              >
                Out Stock
              </span>
              <p
                className={`text-lg font-extrabold ${activeFilter === 'out' ? 'text-gray-900' : 'text-gray-700'}`}
              >
                {stats.outOfStock}
              </p>
              {activeFilter === 'out' && (
